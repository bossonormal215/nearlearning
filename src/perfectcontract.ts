...
class FungibleToken {
  ...
  init({ owner_id }: { owner_id: AccountId }): void {
    if (this.initialized) {
      near.log(`Contract Already Initialized to ${this.get_owner}`);
    ...
    near.log(`Contract initialized for ${owner_id}`);
  }
  ...
  ft_transfer({
    ...
    const sender_balance = Number(this.balances.get(sender_id)) || 0;
    const amount_bigint = Number(amount);
    ...
    near.promiseBatchActionTransfer(promise, BigInt(amount_bigint));

    this.balances.set(sender_id, (sender_balance - amount_bigint).toString());
    const receiver_balance = Number(this.balances.get(receiver_id) || 0);
    this.balances.set(
      receiver_id,
    ...
    // Log the transfer
    near.log(
      `Transfer ${amount} from ${sender_id} to ${receiver_id} memo ${memo}`
    );
  ...
  @view({})
  getAccountStorageCost(): string {
    const storageBytes = 128;
    const costPerByte = BigInt(near.storageByteCost());
    return (BigInt(storageBytes) * costPerByte).toString();
  }
  ...
  storage_deposit({ account_id }: { account_id?: AccountId }): {
    ...
    const requiredDeposit = BigInt(this.getAccountStorageCost());
    const attachedDeposit = near.attachedDeposit();

    if (BigInt(attachedDeposit) < requiredDeposit) {
      throw new Error(
        `Insufficient deposit. Required: ${requiredDeposit}, Attached: ${attachedDeposit}`
      );
    }
